...
purge.keyvaults: ## Purge keyvaults using azure CLI
	@echo -e "${PURPLE}Running target '$@'${NC}"
	/bin/bash -c \
	"for i in \`az keyvault list-deleted -o tsv --query \"[?tags.environment=='$(ENVIRONMENT)'].name\"\`; do az keyvault purge --name \$$i; done"

purge.resource-groups: ## Purge resource groups using azure CLI
	@echo -e "${PURPLE}Running target '$@'${NC}"
	/bin/bash -c \
    "for i in \`az group list -o tsv --query \"[?tags.environment=='$(ENVIRONMENT)'].name\"\`; do echo \"purging resource group: \$$i\" && \$$(az group delete -n \$$i -y --no-wait || true); done"
